...
public class DroneProxy {
	...
	public enum DroneProgressiveCommandFlag {
		ARDRONE_PROGRESSIVE_CMD_ENABLE, // 1: use progressive commands - 0: try
										// hovering
		ARDRONE_PROGRESSIVE_CMD_COMBINED_YAW_ACTIVE, // 1: activate combined yaw
														// - 0: Deactivate
														// combined yaw
		ARDRONE_MAGNETO_CMD_ENABLE, // 1: activate the magneto piloting mode -
									// 0: desactivate the mode
	};
	...
	// This enum should match the typedef VIDEO_RECORDING_CAPABILITY in
	// jni/API/common.h
	...
	/**
	 * Broadcast action: This will be sent in response to {@link doConnect()}
	 * when drone proxy is connected to the ARDroneLib and ARDrone lib is fully
	 * initialized and ready.
	 */
	public static final String DRONE_PROXY_CONNECTED_ACTION = "drone.proxy.connected.action";

	/**
	 * Broadcast action: Will be sent in response to {@link doDisconnect()} when
	 * drone proxy is disconnected and ARDroneLib is released.
	 */
	public static final String DRONE_PROXY_DISCONNECTED_ACTION = "drone.proxy.disconnected.action";

	/**
	 * Broadcast action: Will be sent in response to {@link
	 * setDefaultConfigurationNative()} or {@link triggerConfigUpdateNative()}
	 * if drone configuration has changed.
	 */
	public static final String DRONE_PROXY_CONFIG_CHANGED_ACTION = "drone.proxy.config.changed.action";

	/**
	 * Broadcast action: Will be sent if error occured during connection to the
	 * AR.Drone.
	 */
	...
	public void doConnect(final Context context,
			EVideoRecorderCapability recordVideoResolution) {
		try {
			Log.d(TAG, "Connecting...");

			String uid = Secure.getString(context.getContentResolver(),
					Secure.ANDROID_ID);
			...
			if (mediaDir == null) {
				// This is really bad. We can't get directory to cache media
				// files.
				Log.w(TAG, "Cache/Media dir is unavailable.");
				connect(packageName.trim(), uid.trim(),
						cacheDir.getAbsolutePath(), cacheDir.getAbsolutePath(),
						0, EVideoRecorderCapability.NOT_SUPPORTED.ordinal());
			} else {
				...
				int spaceToUse = (int) Math.round(usableSpace
						* ((double) ApplicationSettings.MEMORY_USAGE / 100.0)
						/ 1048576.0);

				connect(packageName.trim(), uid.trim(),
						cacheDir.getAbsolutePath(), mediaDir.getAbsolutePath(),
	...
	public void onConnected() {
		...
		LocalBroadcastManager mgr = LocalBroadcastManager
	...
	/*
	 * Rename with caution! This method is called from native code. Rename will
	 * require rebuild of the native libraries!
	 */
	public void onAcademyNewMediaReady(String path, boolean addToQueue) {
		if (null == path || 0 == path.length()) {
			...
			if (file.exists() && file.isFile()) {
				...
				if (!file.exists()) {
					Log.w(TAG, "File " + path
							+ " doesn't exists but reported as new media");
				}
			}
	...
	public native void playLedAnimation(float frequency, int duration,
			int animationMode);
	...
	private native void connect(String appName, String username,
			String rootDir, String flightDir, int flightSize,
